...
private:
	...
	string getMiningSubmitString(unsigned int jobId, unsigned int timestamp, unsigned int nonce) {
		picojson::object top;
	...
	ServerCommand getCommandType(string line) {
		...
		if (!err.empty()) {
			sprintf(outputBuffer, "An error has been encountered while reading a command! Command: %s", line.c_str());
		...
		if (!jsonMaster.is<picojson::object>()) {
			sprintf(outputBuffer, "Top-level line %s is not a JSON object!", line.c_str());
			cerr << outputBuffer << endl;
			Log::error(outputBuffer);
			return Invalid;
		}

		const picojson::value::object& jsonMasterObj = jsonMaster.get<picojson::object>();

		picojson::value::object::const_iterator jsonCommandIterator = jsonMasterObj.find("command");
	...
	bool setupLoginAndSubscribe(string username, string password) {
...
#ifdef _WIN32
			sprintf(outputBuffer, "Reading from socket during setup resulted in an error %d", WSAGetLastError());
			cerr << outputBuffer << endl;
			Log::error(outputBuffer);
		...
		if (getCommandType(message) != Capabilities) {
			sprintf(outputBuffer, "Server did not send its capabilities at the beginning of the setup process! Instead, it sent the command: %s", message);
			cerr << outputBuffer << endl;
			Log::error(outputBuffer);
			return false;
		}
		...
		if (!err.empty()) {
			sprintf(outputBuffer, "An error has occurred while attempting to read the server capabilities: %s", err.c_str());
			cerr << outputBuffer << endl;
			Log::error(outputBuffer);
			return false;
		}
		...
		if (!capabilitiesCommand.is<picojson::object>()) {
			sprintf(outputBuffer, "Provided JSON (%s) does not contain a top-level JSON-object!", message);
			cerr << outputBuffer << endl;
			Log::error(outputBuffer);
		}

		const picojson::value::object& capabilitiesCommandObj = capabilitiesCommand.get<picojson::object>();

		picojson::value::object::const_iterator capabilitiesIter = capabilitiesCommandObj.find("capabilities");

		if (capabilitiesIter != capabilitiesCommandObj.end()) {
			picojson::value capabilitiesSection = capabilitiesIter->second;
			const picojson::value::object& capabilitiesObject = capabilitiesSection.get<picojson::object>();
			picojson::value::object::const_iterator capabilitiesInternalIter = capabilitiesObject.find("data");
			if (capabilitiesInternalIter != capabilitiesObject.end()) {
				...
				char MINING_SUBSCRIBE = bitflag[bitflag.length() - 1 - MINING_SUBSCRIBE_OFFSET];
				char MINING_SUBMIT = bitflag[bitflag.length() - 1 - MINING_SUBMIT_OFFSET];
				char MINING_UNSUBSCRIBE = bitflag[bitflag.length() - 1 - MINING_UNSUBSCRIBE_OFFSET];
				char MINING_RESET_ACK = bitflag[bitflag.length() - 1 - MINING_RESET_ACK_OFFSET];
				char MINING_MEMPOOL_UPDATE_ACK = bitflag[bitflag.length() - 1 - MINING_MEMPOOL_UPDATE_ACK_OFFSET];

				boolean capabilitiesCorrect = true;

				if (MINING_AUTH != '1') {
					capabilitiesCorrect = false;
					sprintf(outputBuffer, "The specified server does not support MINING_AUTH according to its bitflag (%s)", bitflag.c_str());
					cerr << outputBuffer << endl;
					Log::error(outputBuffer);
				}
				...
				if (MINING_SUBSCRIBE != '1') {
					capabilitiesCorrect = false;
					sprintf(outputBuffer, "The specified server does not support MINING_SUBSCRIBE according to its bitflag (%s)", bitflag.c_str());
					cerr << outputBuffer << endl;
					Log::error(outputBuffer);
				}
				...
				if (MINING_SUBMIT != '1') {
					capabilitiesCorrect = false;
					sprintf(outputBuffer, "The specified server does not support MINING_SUBMIT according to its bitflag (%s)", bitflag.c_str());
					cerr << outputBuffer << endl;
					Log::error(outputBuffer);
				}
				...
				if (MINING_UNSUBSCRIBE != '1') {
					capabilitiesCorrect = false;
					sprintf(outputBuffer, "The specified server does not support MINING_UNSUBSCRIBE according to its bitflag (%s)", bitflag.c_str());
					cerr << outputBuffer << endl;
					Log::error(outputBuffer);
				}
				...
				if (MINING_RESET_ACK != '1') {
					capabilitiesCorrect = false;
					sprintf(outputBuffer, "The specified server does not support MINING_RESET_ACK according to its bitflag (%s)", bitflag.c_str());
					cerr << outputBuffer << endl;
					Log::error(outputBuffer);
				}
				...
				if (MINING_MEMPOOL_UPDATE_ACK != '1') {
					capabilitiesCorrect = false;
					sprintf(outputBuffer, "The specified server does not support MINING_MEMPOOL_UPDATE_ACK according to its bitflag (%s)", bitflag.c_str());
					cerr << outputBuffer << endl;
					Log::error(outputBuffer);
				}

				if (capabilitiesCorrect) {
					sprintf(outputBuffer, "The specified server supports all necessary commands (bitflag: %s)", bitflag.c_str());
					cout << outputBuffer << endl;
				...
				sprintf(outputBuffer, "The server did not send a valid capabilities command!");
				cerr << outputBuffer << endl;
				Log::error(outputBuffer);
				return false;
			}
		...
		success = send(ucpServerSocket, authenticate.c_str(), (int)strlen(authenticate.c_str()), 0);

		if (success == SOCKET_ERROR) {
#ifdef _WIN32
			sprintf(outputBuffer, "Sending authentication string failed with error %d", WSAGetLastError());
			cerr << outputBuffer << endl;
			Log::error(outputBuffer);
			closesocket(ucpServerSocket);
			WSACleanup();
#else
			sprintf(outputBuffer, "Sending authentication string failed with error %d", errno);
			cerr << outputBuffer << endl;
			Log::error(outputBuffer);
			close(ucpServerSocket);
#endif
		...
		if (success == SOCKET_ERROR) {
#ifdef _WIN32
			sprintf(outputBuffer, "Reading from socket during authentication resulted in an error %d", WSAGetLastError());
			cerr << outputBuffer << endl;
			Log::error(outputBuffer);
			closesocket(ucpServerSocket);
			WSACleanup();
#else
			sprintf(outputBuffer, "Reading from socket during authentication resulted in an error %d", errno);
			cerr << outputBuffer << endl;
			Log::error(outputBuffer);
			close(ucpServerSocket);
#endif
		...
		if (getCommandType(message) == MiningAuthSuccess) {
			...
			if (!err.empty()) {
				sprintf(outputBuffer, "An error has occurred while attempting to read the server authentication response: %s", err.c_str());
				cerr << outputBuffer << endl;
				Log::error(outputBuffer);
				return false;
			}
			...
			if (!authenticationResponseCommand.is<picojson::object>()) {
				sprintf(outputBuffer, "Provided JSON (%s) does not contain a top-level JSON-object!", err.c_str());
				cerr << outputBuffer << endl;
				Log::error(outputBuffer);
				return false;
			}

			const picojson::value::object& authenticationResponseCommandObj = authenticationResponseCommand.get<picojson::object>();

			picojson::value::object::const_iterator authenticationResponseIter = authenticationResponseCommandObj.find("reason");

			if (authenticationResponseIter != authenticationResponseCommandObj.end()) {
				picojson::value reasonValue = authenticationResponseIter->second;
				string reason = reasonValue.to_str();
				sprintf(outputBuffer, "Unable to authenticate to the server: %s", reason.c_str());
				cerr << outputBuffer << endl;
				Log::error(outputBuffer);
				return false;
			}
			else {
				sprintf(outputBuffer, "The server did not send a valid mining authentication response command!");
				cerr << outputBuffer << endl;
				Log::error(outputBuffer);
				return false;
			}
		...
		success = send(ucpServerSocket, subscribe.c_str(), (int)strlen(subscribe.c_str()), 0);

		if (success == SOCKET_ERROR) {
#ifdef _WIN32
			sprintf(outputBuffer, "Sending subscription string failed with error %d", WSAGetLastError());
			cerr << outputBuffer << endl;
			Log::error(outputBuffer);
			closesocket(ucpServerSocket);
			WSACleanup();
#else
			sprintf(outputBuffer, "Sending subscription string failed with error %d", errno);
			cerr << outputBuffer << endl;
			Log::error(outputBuffer);
			close(ucpServerSocket);
#endif
		...
		if (success == SOCKET_ERROR) {
#ifdef _WIN32
			sprintf(outputBuffer, "Reading from socket during subscription resulted in an error %d", WSAGetLastError());
			cerr << outputBuffer << endl;
			Log::error(outputBuffer);
			closesocket(ucpServerSocket);
			WSACleanup();
#else
			sprintf(outputBuffer, "Reading from socket during subscription resulted in an error %d", errno);
			cerr << outputBuffer << endl;
			Log::error(outputBuffer);
			close(ucpServerSocket);
#endif
		...
		if (getCommandType(message) == MiningSubscribeSuccess) {
			...
			if (!err.empty()) {
				sprintf(outputBuffer, "Reading from socket during subscription resulted in an error %s", err.c_str());
				cerr << outputBuffer << endl;
				Log::error(outputBuffer);
				return false;
			}
			...
			const picojson::value::object& subscriptionResponseCommandObj = subscriptionResponseCommand.get<picojson::object>();

			picojson::value::object::const_iterator subscriptionResponseIter = subscriptionResponseCommandObj.find("reason");

			if (subscriptionResponseIter != subscriptionResponseCommandObj.end()) {
				picojson::value reasonValue = subscriptionResponseIter->second;
				string reason = reasonValue.to_str();
				sprintf(outputBuffer, "Unable to subscribe to server: %s", reason.c_str());
				cerr << outputBuffer << endl;
				Log::error(outputBuffer);
				return false;
			}
			else {
				sprintf(outputBuffer, "The server did not send a valid mining subscription response command!");
				cerr << outputBuffer << endl;
				Log::error(outputBuffer);
				return false;
			}
	...
	picojson::value extractDataValueFromJSONById(string JSON, string id) {
		...
		if (!err.empty()) {
			sprintf(outputBuffer, "An error has occurred while attempting to read the server response: %s", err.c_str());
			cerr << outputBuffer << endl;
			Log::error(outputBuffer);
		...
		if (!command.is<picojson::object>()) {
			sprintf(outputBuffer, "Provided JSON (%s) does not contain a top-level JSON-object!", JSON.c_str());
			cerr << outputBuffer << endl;
			Log::error(outputBuffer);
			throw invalid_argument("Provided JSON (" + JSON + ") does not contain an object on its top level!");
		}
		...
		if (jobIdIter != commandObj.end()) {
			...
			const picojson::value::object& jobIdSectionObject = jobIdSection.get<picojson::object>();
			picojson::value::object::const_iterator jobIdInternalIter = jobIdSectionObject.find("data");
			...
			if (jobIdInternalIter != jobIdSectionObject.end()) {
				...
				sprintf(outputBuffer, "The JSON blob (%s) does not contain an id %s!", JSON.c_str(), id.c_str());
				cerr << outputBuffer << endl;
				Log::error(outputBuffer);
				throw invalid_argument("The JSON blob (" + JSON + ") does not contain an id " + id + "!");
			}
		}
		else {
			sprintf(outputBuffer, "The JSON blob (%s) does not contain an id %s!", JSON.c_str(), id.c_str());
	...
	int getValueFromLowerCaseHex(char toRoute) {
		if (!isLowerCaseHexCharacter(toRoute)) {
			throw invalid_argument("The provided character " + string(1, toRoute) + " is not valid!");
		}
		...
		if (toRoute >= 48 && toRoute <= 57) {
			...
			throw invalid_argument("The provided character " + string(1, toRoute) + " is invalid and was not rejected in preliminary hex checks!");
		}
	}
	...
	byte extractByteFromHex(string hex, int byteIndex) {
		...
		if (!isLowerCaseHexCharacter(hi)) {
			throw invalid_argument("Hex character " + string(1, hi) + " is not valid!");
		}
		if (!isLowerCaseHexCharacter(lo)) {
			throw invalid_argument("Hex character " + string(1, lo) + " is not valid!");
		}
		...
		if (hiVal + loVal > 255) {
			throw invalid_argument("The provided hex (" + hex + ") at index " + to_string(byteIndex) + " is not a valid byte!");
		}
	...
	void cyclicRun() {
		...
		for (;;) {
			...
			if ((!check1)) {
				Log::info("Message from server was chopped into multiple packets, reading additional packets...");
			}
			...
			if (success == SOCKET_ERROR) {
#ifdef _WIN32
				sprintf(outputBuffer, "Reading from socket during normal operations resulted in an error %d, are the server credentials correct?", WSAGetLastError());
				cerr << outputBuffer << endl;
				Log::error(outputBuffer);
				closesocket(ucpServerSocket);
				WSACleanup();
#else
				sprintf(outputBuffer, "Reading from socket during normal operations resulted in an error %d, are the server credentials correct?", errno);
				cerr << outputBuffer << endl;
				Log::error(outputBuffer);
				close(ucpServerSocket);
#endif
			...
			if (commandType == MiningJob) {
				...
				byte secondPreviousBlockHashBytes[SECOND_PREVIOUS_BLOCK_HASH_SIZE_BYTES];
				...
				previousBlockHash = getDataStringFromJSONById(message, "previous_block_hash");
				for (int i = BLOCK_HASH_SIZE_BYTES - PREVIOUS_BLOCK_HASH_SIZE_BYTES; i < BLOCK_HASH_SIZE_BYTES; i++) {
					previousBlockHashBytes[i - (BLOCK_HASH_SIZE_BYTES - PREVIOUS_BLOCK_HASH_SIZE_BYTES)] = extractByteFromHex(previousBlockHash, i * 2);
				}

				secondPreviousBlockHash = getDataStringFromJSONById(message, "second_previous_block_hash");
				for (int i = BLOCK_HASH_SIZE_BYTES - SECOND_PREVIOUS_BLOCK_HASH_SIZE_BYTES; i < BLOCK_HASH_SIZE_BYTES; i++) {
					secondPreviousBlockHashBytes[i - (BLOCK_HASH_SIZE_BYTES - SECOND_PREVIOUS_BLOCK_HASH_SIZE_BYTES)] = extractByteFromHex(secondPreviousBlockHash, i * 2);
				}
				thirdPreviousBlockHash = getDataStringFromJSONById(message, "third_previous_block_hash");
				for (int i = BLOCK_HASH_SIZE_BYTES - THIRD_PREVIOUS_BLOCK_HASH_SIZE_BYTES; i < BLOCK_HASH_SIZE_BYTES; i++) {
					thirdPreviousBlockHashBytes[i - (BLOCK_HASH_SIZE_BYTES - THIRD_PREVIOUS_BLOCK_HASH_SIZE_BYTES)] = extractByteFromHex(thirdPreviousBlockHash, i * 2);
				}
				...
				string miningTargetHex = getDataStringFromJSONById(message, "mining_target");
				...
				memcpy(headerToHash + 6, previousBlockHashBytes, PREVIOUS_BLOCK_HASH_SIZE_BYTES);
				memcpy(headerToHash + 18, secondPreviousBlockHashBytes, SECOND_PREVIOUS_BLOCK_HASH_SIZE_BYTES);
				memcpy(headerToHash + 27, thirdPreviousBlockHashBytes, THIRD_PREVIOUS_BLOCK_HASH_SIZE_BYTES);
				memcpy(headerToHash + 36, topLevelMerkleRootBytes, TOP_LEVEL_MERKLE_ROOT_SIZE_BYTES);
				...
				string topLevelMerkleRootHex = getDataStringFromJSONById(message, "new_merkle_root");
				for (int i = 0; i < TOP_LEVEL_MERKLE_ROOT_SIZE_BYTES; i++) {
					topLevelMerkleRoot[i] = extractByteFromHex(topLevelMerkleRootHex, i * 2);
				}

				memcpy(headerToHash + 36, topLevelMerkleRoot, TOP_LEVEL_MERKLE_ROOT_SIZE_BYTES);
				...
				sprintf(outputBuffer, "Submitting a share failed for the following reason: %s", failureReason.c_str());
				cerr << outputBuffer << endl;
				lastAcknowledgement = sentShares;
				Log::error(outputBuffer);
			}
...
public:
	...
	unsigned int getEncodedDifficulty() {
		return encodedDifficulty;
	}

	unsigned long long getStartExtraNonce() {
		return startExtraNonce;
	}
	...
	void submitWork(unsigned int jobId, unsigned int timestamp, unsigned int nonce) {
		string miningSubmit = getMiningSubmitString(jobId, timestamp, nonce) + "\n";
		long success = send(ucpServerSocket, miningSubmit.c_str(), (int)strlen(miningSubmit.c_str()), 0);
		...
		if (success == SOCKET_ERROR) {
#ifdef _WIN32
			sprintf(outputBuffer, "Sending mining submit / share submission string failed with error %d", WSAGetLastError());
			cerr << outputBuffer << endl;
			Log::error(outputBuffer);
			closesocket(ucpServerSocket);
			WSACleanup();
#else
			sprintf(outputBuffer, "Sending mining submit / share submission string failed with error %d", errno);
			cerr << outputBuffer << endl;
			Log::error(outputBuffer);
			close(ucpServerSocket);
#endif
	...
	bool connectToServer(bool startThread) {
		...
		bool successfulStart = UCPClient::setupLoginAndSubscribe(storedUsername, storedPassword);
